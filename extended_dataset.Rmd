...
search_metric(dougscore, "ferrari", "hp", max_results = 3)
```
...
```{r}
plot_score_vs_spec(dougscore, x = "mpg", y = "daily_score")
```
...
###Test: Easter Egg
```{r}
generate_doug_joke(dougscore, "Hummer EV")
```
...
```{r}
compare_cars_llm(dougscore, "ferrari f40", "ferrari f50")
```
